"""Define a set of geometric data augmentation which can be applied to the input image and its corresponding annotations.
This is relevant for the task of semantic segmentation since the input image and its annotation need to be treated in
the same way.
"""
...
class GeometricDataAugmentation(ABC):
    ...
    @abstractmethod
    def __call__(self, image: Tensor, anno: Tensor) -> Tuple[Tensor, Tensor]:
        """ Apply a geometric transformation to a given image and its corresponding annotation.

        Args:
            image(Tensor): input image to be transformed.
            anno(Tensor): annotation to be transformed.

        Returns:
            Tuple[Tensor, Tensor]: transformed image and its corresponding annotation
        """
...
class RandomTranslationTransform(GeometricDataAugmentation):
    ...
    def __call__(self, image: Tensor, anno: Tensor) -> Tuple[Tensor, Tensor]:
        assert image.shape[1] == anno.shape[1], "Dimension of all input should be identical."
        assert image.shape[2] == anno.shape[2], "Dimension of all input should be identical."
        ...
        image_translated = functional.affine(image, angle=0, translate=[tx, ty], scale=1.0, shear=[0, 0],
                                             interpolation=transforms.InterpolationMode.BILINEAR)
        anno_translated = functional.affine(anno, angle=0, translate=[tx, ty], scale=1.0, shear=[0, 0],
...
class RandomRotationTransform(GeometricDataAugmentation):
    ...
    def __call__(self, image: Tensor, anno: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        anno_rorated = functional.rotate(anno, angle=angle, interpolation=transforms.InterpolationMode.NEAREST)

        return image_rotated, anno_rorated


class RandomGaussianRotationTransform(GeometricDataAugmentation):
    """Randomly rorate an image and its annotation by a random angle specified by a gaussian.

     Args:
            mean(float, optional): mean rotation angle in radians. Defaults to 0.0.
            std(float, optional): std rotation angle in radians. Defaults to 0.0.
    """
    ...
    def __call__(self, image: Tensor, anno: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        angle = random.gauss(self.mean, self.std)
...
class RandomHorizontalFlipTransform(GeometricDataAugmentation):
    ...
    Args:
        prob(float, optional): probability of the image being flipped. Default to 0.5.
    """
...
class CenterCropTransform(GeometricDataAugmentation):
    """Set height and width of cropping center.
    ...
    def __call__(self, image: Tensor, anno: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        assert image_cropped.shape[1] == img_height, "Cropped image has an unexpected number of height."
        assert image_cropped.shape[2] == img_width, "Cropped image has an unexpected number of width."

        assert anno_cropped.shape[0] == anno_chans, "Cropped image has an unexpected number of channels."
        assert anno_cropped.shape[1] == self.crop_height, "Cropped image has an unexpected number of height."
        assert anno_cropped.shape[2] == self.crop_width, "Cropped image has an unexpected number of width."
...
class RandomCropTransform(GeometricDataAugmentation):
    ...
    def __call__(self, image: Tensor, anno: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        if self.crop_width > img_width:
            raise ValueError(
                f"Width of cropping region must not be greater than img width: {self.crop_width} vs {img_width}")
        if self.crop_height > img_height:
            raise ValueError(
                f"Height of cropping region must not be greater than img height: {self.crop_height} vs {img_height}")
...
class MyRandomScaleTransform(GeometricDataAugmentation):
    ...
    def __call__(self, image: Tensor, anno: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        if random.random() < self.prob:
            ...
            image = functional.affine(image, angle=0, translate=[0, 0], scale=random_scale, shear=[0, 0],
                                      interpolation=transforms.InterpolationMode.BILINEAR)
            anno = functional.affine(anno, angle=0, translate=[0, 0], scale=random_scale, shear=[0, 0],
                                     interpolation=transforms.InterpolationMode.NEAREST)
...
class MyRandomAspectRatioTransform(GeometricDataAugmentation):
    """Rescaling the image and its annotation in one dimension (width or height) to diversity the aspect ratio.
       This augmentation is proposed in: http://www.robesafe.es/personal/eduardo.romera/pdfs/Romera18iv.pdf

    Args:
        min_ap_scale(float): minimum scaling factor.
        max_ap_scale(float): maximum scaling factor.
        prob(float, optional): probability of the image being scaled. Defaults to 0.5.
    """

    def __init__(self, min_ap_scale: float, max_ap_scale: float, prob: float = 0.5):
        assert prob >= 0.0
        assert prob <= 1.0
        assert min_ap_scale > 0.0
        assert max_ap_scale >= min_ap_scale

        self.min_ap_scale = min_ap_scale
        self.max_ap_scale = min_ap_scale
        self.prob = prob
    ...
    def __call__(self, image: Tensor, anno: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        if random.random() < self.prob:
            random_scale = random.uniform(self.min_ap_scale, self.max_ap_scale)
            ...
            image = functional.resize(image, size=[img_height_new, img_width_new],
                                      interpolation=transforms.InterpolationMode.BILINEAR)
            anno = functional.resize(anno, size=[img_height_new, img_width_new],
                                     interpolation=transforms.InterpolationMode.NEAREST)
...
class MyRandomShearTransform(GeometricDataAugmentation):
    ...
    Args:
        max_x_shear(float): maximum shear along x-axis(in degrees).
        max_y_shear(float): maximum shear along y-axis(in degrees).
        prob(float,optional): probability of the image being sheared. Default to 0.5.
    """
    ...
    def __call__(self, image: Tensor, anno: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        if random.random() < self.prob:
            ...
            image = functional.affine(image, angle=0, translate=[0, 0], scale=1.0, shear=[x_shear, 0],
                                      interpolation=transforms.InterpolationMode.BILINEAR)
            anno = functional.affine(anno, angle=0, translate=[0, 0], scale=1.0, shear=[x_shear, 0],
                                     interpolation=transforms.InterpolationMode.NEAREST)

        if random.random() < self.prob:
            y_shear = random.uniform(-self.max_y_shear, self.max_y_shear)

            image = functional.affine(image, angle=0, translate=[0, 0], scale=1.0, shear=[y_shear, 0],
                                      interpolation=transforms.InterpolationMode.BILINEAR)
            anno = functional.affine(anno, angle=0, translate=[0, 0], scale=1.0, shear=[y_shear, 0],
                                     interpolation=transforms.InterpolationMode.NEAREST)
...
def get_geometric_augmentations(cfg, stage: str) -> List[GeometricDataAugmentation]:
    ...
    for tf_name in cfg[stage]['geometric_data_augmentations'].keys():
        ...
        if tf_name == 'random_scale':
            ...
            max_scale = cfg[stage]['geometric_data_augmentations'][tf_name]['min_scale']
        ...
        if tf_name == 'random_aspect_ratio':
            ...
            max_ap_scale = cfg[stage]['geometric_data_augmentations'][tf_name]['max_cale']
