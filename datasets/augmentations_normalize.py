...
class NoImageNormalizer(ImageNormalizer):
    """ Do not perform any normalization the image
    """
...
class SingleImageNormalizer(ImageNormalizer):
    ...
    def normalize(self, image: torch.Tensor) -> torch.Tensor:
        ...
        # get channels
        r_chan, g_chan, b_chan = image_rgb_split(image)

        # get channel stats
        r_mean, r_std, g_mean, g_std, b_mean, b_std = image_stats(image)
        ...
        image_normalized = torch.stack([r_chan_normed, g_chan_normed, b_chan_normed], dim=0)
