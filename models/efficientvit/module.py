...
class ConvLayer(nn.Module):
    def __init__(self,
        ...
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias=use_bias)
    ...
    def __init__(self,
        ...
        self.aggreg = nn.ModuleList([nn.Sequential(
            nn.Conv2d(3 * total_dim, 3 * total_dim, scale, padding=get_same_padding(scale), groups=3 * total_dim,
                      bias=use_bias[0]),
            nn.Conv2d(3 * total_dim, 3 * total_dim, 1, groups=3 * heads, bias=use_bias[0])) for scale in scales])
