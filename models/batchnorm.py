...
class myLayerNorm(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        x = x.flatten(2).transpose(1, 2)  # BCHW -> BC * HW -> B*HW*C
        x = self.norm(x)
...
class _SynchronizedBatchNorm(_BatchNorm):
    ...
    def __data_parallel_replicate_(self, ctx, copy_id):
...
norm_layer = partial(SynchronizedBatchNorm2d, momentum=float(1e-4))


class NormLayer(nn.Module):
    def __init__(self, inChannels, norm_type):
        super().__init__()
