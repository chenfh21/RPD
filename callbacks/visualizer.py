...
def blend_images(image1: np.ndarray, image2: np.ndarray, alpha: float = 0.5) -> np.ndarray:
    ...
    Args:
        ...
        alpha (float, optional): strength of blending for 1st image. Default to 0.5.
...
class SemanticOverlayCorrectIncorrectVisualizer(BasicVisualizer):
    """ Basic representation of a visualizer. """
...
def get_visualizers(cfg: Dict) -> List[BasicVisualizer]:
    ...
    for visualizer_name in cfg['visualizers'].keys():
        ...
        if visualizer_name == 'semantic_overlay_correct_incorrect_visualizer':
            classes_to_colors = cfg['visualizers'][visualizer_name]['classes_to_colors']
            visualizer = SemanticOverlayCorrectIncorrectVisualizer(classes_to_colors)
            visualizers.append(visualizer)
...
class VisualizerCallback(Callback):
    ...
    def __init__(self, visualizers: List[BasicVisualizer], vis_train_every_x_epochs: int = 1,
                 vis_val_every_x_epochs: int = 1):
        """ Constructor.
    ...
    """
        super(VisualizerCallback).__init__()
