...
class Postprocessor(ABC):
    ...
    def process_embeddings(self, embeddings: torch.Tensor) -> Union[torch.Tensor, None]:
        """ Perform a post-processing of the embeddings computed by the network.
    ...
    @abstractmethod
    def save_postprocessed_logits(self, processed_logits: Union[torch.Tensor, None], path_to_dir: str,
    ...
    @abstractmethod
    def save_postprocessed_embeddings(self, processed_embeddings: Union[torch.Tensor, None], path_to_dir: str,
                                      fnames: List[str]) -> None:
        """ Save processed outputs created by this class.

        Args:
            processed_embeddings(torch.Tensor): post-processed output of shape [batch_size x ? x H x W](? := depends on implementation)
            path_to_dir(str): path to output directory
            fnames(List[str]): raw filenames with fileformat (len(fnames)==batch_size)
        """
        raise NotImplementedError
...
class KeepLogitsPostprocessor(Postprocessor):
    ...
    def save_postprocessed_logits(self, processed_logits: Union[torch.Tensor, None], path_to_dir: str,
        ...
        if not (processed_logits.device == torch.device('cpu')):
            processed_logits = processed_logits.cpu()
...
class ProbablisticSoftmaxPostprocessor(Postprocessor):
    ...
    def save_postprocessed_logits(self, processed_logits: Union[torch.Tensor, None], path_to_dir: str,
                                  fnames: List[str]) -> None:
        """ Save predicted probabilities for each image as a tiff image.
         Args:
...
def get_postprocessors(cfg: Dict) -> List[Postprocessor]:
    ...
    for postprocessors_name in cfg['postprocessors'].keys():
        ...
        if postprocessors_name == 'arg_max_class_postprocessor':
...
class PostprocessorCallback(Callback):
    ...
    def __init__(self, postprocessors: List[Postprocessor], postprocess_train_every_x_epochs: int = 1,
                 postprocess_val_every_x_epochs: int = 1):
        """ Constructor.

        Args:
            postprocess_train_every_x_epochs(int): Frequency of train postprocessing. Defaults to 1.
            postprocess_val_every_x_epochs(int): Frequency of val postprocessing. Default to 1.
        """
        super(PostprocessorCallback, self).__init__()
